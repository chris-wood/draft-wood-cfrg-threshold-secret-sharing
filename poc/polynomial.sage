...
def derive_poylnomial_coefficient(F, r, t, index, dst):
    ctx = bytes(dst) + _as_bytes("-") + F.serialize_scalar(t) + _as_bytes("-") + F.serialize_scalar(index)
    return F.hash_to_scalar(r, ctx)

def derive_poylnomial(F, s, r, t, ctx):
    if t < 2:
        raise Exception("invalid parameters")

    # Construct the polynomial from the random seed and threshold count
    base = derive_poylnomial_coefficient(F, s, t, 0, ctx)
    polynomial_coefficients = [base]
    for i in range(t - 1):
        coefficient = derive_poylnomial_coefficient(F, r, t, i+1, ctx)
